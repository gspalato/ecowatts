...
const ApplianceFinishPage = () => {
	...
	const [name, setName] =
		useState<Database['public']['Tables']['equipment']['Row']['name']>('');
	const [locationId, setLocationId] =
		useState<Database['public']['Tables']['location']['Row']['id']>();
	// Check if the data for the appliance type has the required fields.
	// Otherwise, ask the user to fill in the missing fields.
	const [manuallyFilledFields, setManuallyFilledFields] = useState<{
		[key: string]: any;
	}>({});
	...
	const requiredFields =
		applianceTypeAttributeMap[
			applianceType?.type as keyof typeof applianceTypeAttributeMap
		] ?? {};
	console.log('requiredFields', requiredFields);
	const unfilledRequiredFields = Object.entries(requiredFields).filter(
		([key, value]) => {
			const attribute =
				passedData['atributos'][
					String(value.code) as keyof typeof passedData
				];
			if (
				attribute === undefined ||
				attribute === null ||
							...
							{unfilledRequiredFields.map(([key, value]) => (
								...
								>
									...
									<TextInput
										...
										//placeholder={field.placeholder}
										value={
											manuallyFilledFields[value.code] ||
											''
										}
										onChangeText={(text) => {
						...
						<Button
							...
							onPress={() => {
								...
								const allFieldsFilled =
									unfilledRequiredFields.every(
										([key, { code }]) => {
											return (
												manuallyFilledFields[code] !==
													undefined &&
												manuallyFilledFields[code] !==
													null &&
												manuallyFilledFields[code] !==
